...
class TorneoConfigFrame(tk.CTkFrame):
    ...
    def guardar_data_torneo(self):
        ...
        fecha_inicio = date_to_str(self.fechaIni.get_date())
        fecha_fin = date_to_str(self.fechaFin.get_date())
        ...
        if dt_inicio >= dt_fin:
            CTkMessagebox(title="Error", message="La fecha de inicio no puede ser posterior o igual a la fecha de fin", icon="cancel")
            return
    ...
    def guardar_partidos(self):
        fecha = date_to_str(self.fechaPartido.get_date())
        hora = self.input_partido2.get().strip()
        lugar = self.input_partido3.get().strip()

        if isDateLLenoDePartidos(fecha):
            CTkMessagebox(title="Error", message="La fecha ya esta llena", icon="cancel")
            return
        ...
        isHoraValida, msjValido = horarioValido(hora, fecha)
        if not isHoraValida:
            CTkMessagebox(title="Error", message=msjValido, icon="cancel")
            return
    ...
    def vista1(self):
        ...
        self.input_nombre = tk.CTkEntry(self.frame_torneo, placeholder_text="Nombre del Torneo", width=500)
        ...
        self.fechaIni = datePickerComun(self.frame_torneo)
        self.fechaIni.grid(row=1, column=1, pady=10)

        #self.input_fecha_inicio = tk.CTkEntry(self.frame_torneo, placeholder_text="Fecha de Inicio DD/MM/AAAA", width=200)

        if ini:
            date_ini = str_a_date(ini)
            self.fechaIni.set_date(date_ini)
            self.fechaIni.configure(state="disabled")

        #self.input_fecha_inicio.grid(row=1, column=1, pady=10)
        self.fechaFin = datePickerComun(self.frame_torneo)
        self.fechaFin.grid(row=2, column=1, pady=10)

        #self.input_fecha_fin = tk.CTkEntry(self.frame_torneo, placeholder_text="Fecha de Final DD/MM/AAAA", width=200)

        if fin:
            date_fin = str_a_date(fin)
            self.fechaFin.set_date(date_fin)
            self.fechaFin.configure(state="disabled")
    ...
    def vista4(self):
        ...
        self.fechaPartido = datePickerConRango(self.frame_partidos, rango)
        self.fechaPartido.grid(row=2, column=1, pady=(20, 10))
        ...
        self.input_partido2.bind("<FocusOut>", self.validar_hora_entry) #se activa cuando se quita el focus
    ...
    def validar_hora_entry(self, event=None):
        valor = self.input_partido2.get().strip()
        if not valor:
            return
        try:
            datetime.strptime(valor, "%H:%M")
            self.input_partido2.configure(border_color="gray")
        except ValueError:
            self.input_partido2.configure(border_color="red")
            CTkMessagebox(title="Error", message="Formato de hora inválido, usá hh:mm", icon="cancel")
